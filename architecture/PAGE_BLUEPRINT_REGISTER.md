...
- **Approved appearance:** `CURRENT-01 / VPB-HOME-006` remains the underlying blank-Home composition authority and `CURRENT-02 / VPB-HOME-007` controls selected-path/pre-ASK composition. `VPB-HOME-008/008A/008B` protect the clean responsive environment, `VPB-HOME-009/009A/009B` protect independent REV placement, `VPB-HOME-010` protects the live brand, `VPB-HOME-014` protects the raster pathway family and `VPB-HOME-015` protects the founder-approved desktop blank-state Console Iteration 2 composition. `CURRENT-03 / VPB-HOME-002` and `CURRENT-04 / VPB-HOME-003` remain later understanding/pulse authorities. See `VISUAL_AUTHORITY_REGISTER.md`.
...
- **Current construction control:** Lane 0 governance/checkpoint control; no active production Build Contract. Stage 1D-A Candidate V1 remains `FOUNDER REJECTED — VISUAL DRIFT`. Console Iteration 2 is founder visually approved and byte-locked as `VPB-HOME-015`; this is visual authority only and grants no production implementation. The three independent transparent raster pathway icons remain protected as `VPB-HOME-014`; `VPB-HOME-013` remains unchanged historical approved SVG authority.
...
- **Pathway-icon design boundary:** `VPB-HOME-014` controls one responsive family of three transparent raster decorative assets. Cards, wording, borders, selection controls and accessible naming remain live semantic HTML/CSS; images use proportional contain rendering and remain hidden from the accessibility tree. The SVG-only reconstruction method no longer controls Icons 2 and 3. `VPB-HOME-013` remains protected historical approved authority and is not modified or silently superseded.
- **Stage 1D-B rejection:** Compact Candidate V1 is **FOUNDER REJECTED — RESPONSIVE VISUAL DRIFT** and remains unchanged temporary evidence with SHA-256 `EF803E49C46F1887AFF6BCC0213950C61881B74CB349ADDCAFD7CA3BB967D42D`. It is not authority, fallback or an Iteration 2 source.
- **Stage 1D-B corrected boundary:** A separately authorized candidate may review the same semantic blank-state structure at exactly `1100 × 800`, using `VPB-HOME-008A`, `VPB-HOME-009A`, `VPB-HOME-011` and `VPB-HOME-014` as separate protected layers. `VPB-HOME-015` controls the semantic structure and desktop visual character; `VPB-HOME-006` remains a flattened composition reference only. `X=40–728`, `Y=390–700` (`688 × 310`) is the maximum available envelope, not required filled geometry. Iteration 2 targets approximately `X=40–728`, `Y=390–620` (`688 × 230`) with approximately `62:38` panels: intent `426 × 230`, disabled entry `262 × 230`. Desktop `54:46` pixels do not control compact geometry.
- **Compact hierarchy and controls:** The protected brand remains fixed; hero targets approximately `X=40–690`, `Y=148–280` beneath it on the left; trust targets approximately `X=40–520`, `Y=310–362`. Three cards remain in one row at approximately `129 × 150–158`, with identical approximately `92 × 62` icon-contain boxes. The bottom instruction receives its own live-text lane with at least `8–10px` border clearance. Whole-panel opacity that obscures content is prohibited; disabled heading and description remain readable at approximately `55–65%` strength and genuine disabled controls are subdued individually.
- **Compact clearance:** Console-to-REV clearance remains `81px`; no outer-width or clearance amendment is authorized. Empty transition floor is `Y=620–700`; protected station reserve remains `Y=700–800`; station overlap remains `0px`; REV floor contact remains `X=965`, `Y=610`; receiving-hand target remains `X=826`, `Y=283`.
- **Stage 1D-B approved compact authority:** Compact Iteration 2 is founder visually approved with exact wording `FOUNDER APPROVES STAGE 1D-B COMPACT ITERATION 2` and byte-locked as `VPB-HOME-016`. It protects the `1100 × 800` DPR `1` blank-state composition, console `40,390 · 688 × 230`, `426:262` panels, three `129 × 156` unselected pathway controls, identical `92 × 62` decorative icon-contain boxes, genuine disabled textarea and buttons, protected REV relationship, empty `Y=620–700` transition floor and empty `Y=700–800` station reserve.
- **Compact runtime boundary:** Icons remain unchanged, proportionally contained and accessibility-hidden. Interface wording and controls remain live semantic HTML/CSS. No flattened interface layer, Canvas, script, interaction or production implementation is approved.
- **Compact deferred checks:** Confirm pathway-card descriptions remain readable in the real compact runtime; confirm disabled-panel wording retains adequate contrast across displays; and recheck the visual hand-to-console relationship while preserving `81px` clearance. These checks authorize no automatic change.
- **Compact structure and acceptance gate:** Desktop and compact use one semantic DOM; only responsive CSS sizing, placement, spacing and typography may differ. Both panels stay visible and legible; stacking or structural reflow needs separate founder authorization. The future walkthrough must show REV, its approved contact shadow, live console, visible empty station region and environmental floor together to judge physical grounding. It must also recheck bottom-instruction clearance, disabled right-panel legibility and compact brand-to-hero clearance. These checks authorize no automatic adjustment.
- **Next acceptance test:** `VPB-HOME-016` is visual authority only. Separate founder authorization is required for checkpoint staging/commit or any later candidate or Build Contract. Production, mobile, navigation, interaction, enabled controls, stations, electricity, post-`ASK REV`, Project, persistence, providers, safety processing, Workshop and `REV-FIX-02.5` remain locked.
