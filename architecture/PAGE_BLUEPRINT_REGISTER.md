...
- **Next acceptance test:** A later founder-authorized Build Contract must preserve `VPB-HOME-015` using separate `VPB-HOME-008/009/010/014` and live interface layers, never a flattened runtime image, while keeping the lower station region clear. Live integration must recheck the bottom instruction’s console-edge clearance, disabled right-panel legibility on real displays and the `7px` brand-to-hero clearance during responsive integration. These checks authorize no automatic change; material adjustment requires another founder walkthrough. Production, interaction, stations, electricity, compact, mobile, post-`ASK REV` work and `REV-FIX-02.5` remain locked.
