...
- **Current construction control:** Lane 0 governance/checkpoint control; no active production Build Contract. Stage 1D-A Candidate V1 remains `FOUNDER REJECTED — VISUAL DRIFT`. The three independent transparent raster pathway icons and their in-room card-scale presentation are founder approved and byte-locked as `VPB-HOME-014`. Raster Icon 1 is preferred for this family; `VPB-HOME-013` remains unchanged as historical approved SVG authority. Console Iteration 2 and production remain locked.
...
- **Next acceptance test:** A separately authorized live-integration candidate must place `VPB-HOME-014` within the reconstructed `VPB-HOME-006` console using separate `VPB-HOME-008/009/010` and live interface layers, never a flattened runtime image, while keeping the lower station region clear. Any scale, padding or brightness refinement requires a founder walkthrough; material visual changes require a new candidate authority decision.
